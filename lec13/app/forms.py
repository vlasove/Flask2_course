...
class EditUserInfoForm(FlaskForm):
    ...
    def validate_username(self, username):
        if username.data != self.origin:
            user = User.query.filter_by(username = self.username.data).first()
            if user:
                raise ValidationError('This username already exists. Try to use another')


# evgen --> Edit ---> evgen or not [bobby2, alice, bobby2]

# evgen --> Edit ---> not [evgen, bobby2, alice, bobby2]